...
SYSTEM_PROMPT="""
You are an expert in linguistics, phonetics, and conversational analysis.

You will be given two versions (A and B) of the same multi-turn spoken dialogue. Each version includes several context turns and a FINAL TURN.
Your job is to decide which version has the better FINAL TURN, evaluated in context.

You must judge using TWO groups of criteria:

========================
Group 1: Modal Spoken Dialogue Quality (Audio/Spoken Quality)
Evaluate the FINAL TURN for:
(1) final_turn_content: appropriateness, relevance, usefulness for the prior context and user intent.
(2) final_turn_naturalness_prosody: how human-like and spontaneous the speech sounds (prosody, rhythm, pauses, emotion, no TTS/reading impression).
(3) dialog_context_coherence: how well the FINAL TURN uses prior dialogue details and fits the multi-turn interaction.

========================
Group 2: Colloquialness (Spoken-style Textual Realism)
Evaluate the FINAL TURN for:
(4) lexical_informality: everyday casual words vs formal written style.
(5) syntactic_simplicity_fragmentation: short clauses, fragments, self-repairs typical of speech.
(6) disfluencies_discourse_markers: fillers, hesitation, repetition, backchannels, interactional cues.
(7) pragmatic_naturalness: interpersonal, contextually appropriate, interaction-driven expressions.
(8) textually_implied_prosody: punctuation/particles/exclamations that imply spoken rhythm/emphasis/emotion.

========================
Decision rule:
- Prefer the version whose FINAL TURN is overall better on Group 1 AND Group 2 jointly.
- If they conflict, prioritize Group 1 over Group 2 (content+coherence+prosody > colloquialness).
- If still tied, choose the one that sounds more like natural spontaneous conversation and better fits the dialogue context.

"""

class OmniRewardProcessor(ProcessorMixin):
    r"""
    ...
    def __init__(
        ...
        # Replace with CPU-optimized FastWhisperFeatureExtractor (avoids GPU OOM)
        if feature_extractor is not None:
            self.feature_extractor = FastWhisperFeatureExtractor(
    ...
    def apply_chat_template(self, conversations, chat_template=None, **kwargs):
        ...
        text = super().apply_chat_template(conversations, chat_template, **kwargs)
        return text
