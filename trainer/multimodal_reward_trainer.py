...
from trl.trainer.utils import create_model_from_path, disable_dropout_in_model, get_config_model_id, pad, remove_none_values

from .collator import MultimodalRewardDataCollatorWithPadding, PreprocessedMultimodalDataset
...
class MultimodalRewardTrainer(BaseTrainer):
    """
    ...
    def __init__(
        ...
        # a pad token.
        pad_token = args.pad_token or processing_class.tokenizer.pad_token or processing_class.tokenizer.eos_token
        pad_token_id = processing_class.tokenizer.convert_tokens_to_ids(pad_token)
        ...
        if data_collator is None:
            ...
            data_collator = MultimodalRewardDataCollatorWithPadding(processing_class)
        ...
        # Dataset
        # Wrap datasets with PreprocessedMultimodalDataset to enable multi-process data loading
        # This moves I/O-intensive multimodal processing from collator to dataset __getitem__
        def needs_mm_preprocess(ds):
            ...
            try:
                ...
                return True
            return not ("input_ids_chosen" in sample and "input_ids_rejected" in sample)

        if train_dataset is not None and needs_mm_preprocess(train_dataset):
            train_dataset = PreprocessedMultimodalDataset(train_dataset, use_audio_in_video=True)
        if eval_dataset is not None:
            if isinstance(eval_dataset, dict):
                new_eval_dataset = {}
                for key, dataset in eval_dataset.items():
                    if needs_mm_preprocess(dataset):
                        new_eval_dataset[key] = PreprocessedMultimodalDataset(dataset, use_audio_in_video=True)
                    else:
                        new_eval_dataset[key] = dataset
                eval_dataset = new_eval_dataset
            elif needs_mm_preprocess(eval_dataset):
                eval_dataset = PreprocessedMultimodalDataset(eval_dataset, use_audio_in_video=True)
    ...
    def compute_loss(
        ...
        rewards_chosen = model(
            ...
            input_features=inputs.get("input_features_chosen", None),
            feature_attention_mask=inputs.get("feature_attention_mask_chosen", None),
            pixel_values=inputs.get("pixel_values_chosen", None),
            image_grid_thw=inputs.get("image_grid_thw_chosen", None),
            pixel_values_videos=inputs.get("pixel_values_videos_chosen", None),
            video_grid_thw=inputs.get("video_grid_thw_chosen", None),
            video_second_per_grid=inputs.get("video_second_per_grid_chosen", None),
            return_dict=True,
        )["logits"]
        rewards_rejected = model(
            input_ids=inputs["input_ids_rejected"],
            attention_mask=inputs["attention_mask_rejected"],
            input_features=inputs.get("input_features_rejected", None),
            feature_attention_mask=inputs.get("feature_attention_mask_rejected", None),
            pixel_values=inputs.get("pixel_values_rejected", None),
            image_grid_thw=inputs.get("image_grid_thw_rejected", None),
            pixel_values_videos=inputs.get("pixel_values_videos_rejected", None),
            video_grid_thw=inputs.get("video_grid_thw_rejected", None),
            video_second_per_grid=inputs.get("video_second_per_grid_rejected", None),
            return_dict=True,
        )["logits"]
        ...
        if mode == "train":
            all_attn_mask = torch.cat([inputs["attention_mask_chosen"], inputs["attention_mask_rejected"]], dim=1)
            num_tokens_in_batch = self.accelerator.gather_for_metrics(all_attn_mask.sum()).sum().item()
            self._total_train_tokens += num_tokens_in_batch
        self._metrics[mode]["num_tokens"] = [self._total_train_tokens]

        # Compute min, mean, max, accuracy and margin
        with torch.no_grad():
            all_logits = torch.cat([rewards_chosen, rewards_rejected], dim=0)
            all_rewards = self.accelerator.gather(all_logits)
